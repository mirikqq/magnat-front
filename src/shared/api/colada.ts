...
export function useAppQuery<T>(options: QueryOptions<T>) {
  const nativeUseQuery = (colada as any).useQuery;
  if (nativeUseQuery) {
    return nativeUseQuery(options);
  }
...
export function useAppMutation<TPayload, TResult>(options: MutationOptions<TPayload, TResult>) {
  const nativeUseMutation = (colada as any).useMutation;
  if (nativeUseMutation) {
    return nativeUseMutation(options);
  }
