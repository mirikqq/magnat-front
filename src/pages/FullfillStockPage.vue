<template>
  <section class="page space-y-4">
    ...
    <div class="flex items-center gap-2">
      ...
      <RouterLink class="rounded-md bg-teal-700 px-3 py-2 text-white" to="/fullfill/new?mp=ozon">Новое задание ФФ (OZON)</RouterLink>
    </div>
  </section>
</template>
...
async function save() {
  await observablesService.updateStock({ values: items.value.map((x) => ({ id: x.id, add: draft[x.id] ?? 0 })) });
...
function download() {
  exportRowsToXlsx('fullfill-stock.xlsx', filtered.value.map((x) => ({ Наименование: x.name, Текущие: x.in_our_stock, Закуплено: draft[x.id] ?? 0, Итого: x.in_our_stock + (draft[x.id] ?? 0) })));
}
</script>
