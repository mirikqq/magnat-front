<template>
  <section class="page space-y-4">
    ...
    <div v-if="errorText" class="rounded-2xl border border-red-200 bg-red-50 p-6 text-red-700">{{ errorText }}</div>
    ...
    <template v-else>
      <div class="overflow-auto rounded-md border">
        <table class="min-w-full border-collapse text-sm">
          ...
          <Transition name="table-fade" mode="out-in">
            <TableSkeletonRows v-if="isLoading" key="loading" :columns="8" :rows="4" />
      ...
      <div v-if="!isLoading && !items.length" class="py-10 text-center text-sm text-slate-500">Результаты отсутствуют.</div>

      <div class="flex items-center gap-2">
        <button class="rounded-md border px-2.5 py-1.5 text-sm" :disabled="!order || order.is_approved || isLoading || !items.length" @click="save">Сохранить</button>
        <button class="rounded-md bg-slate-900 px-2.5 py-1.5 text-sm text-white" :disabled="!order || order.is_approved || isLoading || !items.length" @click="approve">Принять</button>
      </div>
    </template>
  </section>
</template>
