<template>
  <section class="page">
    <div class="tile-surface mb-[140px]">
      <div class="mb-4 flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
        <h1 class="text-2xl font-semibold">Потребность</h1>
        <div class="flex flex-wrap items-center gap-2">
          <TransparentButton @click="toggleColumn('name')">Наименование</TransparentButton>
          <TransparentButton @click="toggleColumn('warehouses')">Склады</TransparentButton>
      ...
      <div class="w-full">
        <table class="min-w-full border-collapse text-sm">
          ...
          <Transition name="table-fade" mode="out-in">
            <TableSkeletonRows v-if="showTableSkeleton" key="loading" :columns="table.getVisibleLeafColumns().length || 6" :rows="4" />
    ...
    <div class="fixed bottom-[var(--space-1)] left-[var(--space-1)] right-[var(--space-1)] z-[90] rounded-[var(--radius-base)] border border-white/60 bg-white/94 px-4 py-3 shadow-[0_18px_48px_rgba(15,23,42,0.12)] backdrop-blur-xl">
      <div class="flex flex-wrap items-center justify-between gap-3">
        ...
        <div class="flex items-center gap-2">
          <ColoredButton @click="sendToOrder">Отправить в заказ</ColoredButton>
        </div>
      </div>
    </div>

    <div v-if="stockModalOpen" class="fixed inset-0 z-[200] flex items-center justify-center bg-slate-950/35 p-4" @click.self="closeStockModal">
      <div class="w-full max-w-4xl rounded-[var(--radius-base)] border border-white/60 bg-white p-6 shadow-[0_30px_80px_rgba(15,23,42,0.22)]">
        <div class="mb-4 flex items-center justify-between">
          <h2 class="text-xl font-semibold text-slate-900">Артикул: {{ activeStockMeta.offerId }}</h2>
          <TransparentButton @click="closeStockModal">Закрыть</TransparentButton>
        </div>
        ...
        <div class="w-full">
          <table class="min-w-full border-collapse text-sm">
            ...
            <Transition name="table-fade" mode="out-in">
              <TableSkeletonRows v-if="showStockSkeleton" key="loading" :columns="7" :rows="3" />
              ...
              <tbody v-else-if="stockLoading" key="pending" />
...
watch(
  ...
  (error) => {
    if (error) {
      toast.error('Ошибка загрузки потребности')
    }
  },
)

const showTableSkeleton = useDelayedSkeleton(() => query.isLoading.value)
const showStockSkeleton = useDelayedSkeleton(() => stockLoading.value)
...
async function openStockModal(row: SupplyDemandRowDto) {
  ...
  try {
    ...
    toast.error('Ошибка загрузки остатков по складам')
